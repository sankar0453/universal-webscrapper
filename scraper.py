...
def click_accept_cookies(page: Page):
    ...
    try:
        ...
        page.wait_for_selector("button, a, div", timeout=10000)  # Wait for the presence of button, a, or div tags
        ...
        for tag in ["button", "a", "div"]:
            for text in accept_text_variations:
                try:
                    # Find element by XPath with case-insensitive text matching
                    element = page.locator(f"//{tag}[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), '{text}')]")
                    if element.count() > 0:
                        element.first.click()
...
async def fetch_html_playwright(url):
    async with async_playwright() as p:
        ...
        # await page.set_viewport_size({"width": 1280, "height": 800})  # Set window size to simulate a maximized window

        # Accept cookies (assuming you have a selector for the cookies button)
        # page.click('selector_for_accept_cookies_button')  # Uncomment and set the right selector
        
         # Simulate scrolling
        await page.evaluate("window.scrollTo(0, document.body.scrollHeight);")
        await page.wait_for_timeout(2000)  # Simulate time taken to scroll and read
        await page.evaluate("window.scrollTo(0, document.body.scrollHeight);")
